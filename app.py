...
if page == "1차원 운동":
    ...
    # [핵심] @st.fragment를 사용하여 DuplicateElementId 오류 해결 및 부드러운 재생 구현
    @st.fragment(run_every=0.05 if st.session_state.is_playing else None)
    def render_content(v0, a):
        ...
        # 속도 화살표: 청록색, 두껍고 긴 막대, 작은 머리
        v_scale = curr_v * 1.5 
        fig.add_annotation(
            x=curr_x + v_scale, y=0.8, ax=curr_x, ay=0.8,
        ...
        # 2. 그래프 영역
        def draw_plot(row, col, x, y, f_y, color):
            ...
            fig.add_trace(go.Scatter(x=x, y=y, mode='lines', line=dict(color=color, width=3.5), showlegend=False), row=row, col=col)
            if len(x) > 0:
                fig.add_trace(go.Scatter(x=[x[-1]], y=[y[-1]], mode='markers', marker=dict(color=color, size=8), showlegend=False), row=row, col=col)

        draw_plot(2, 1, t_data, x_data, x_full, "blue")
        draw_plot(2, 2, t_data, v_data, v_full, "green")
        draw_plot(2, 3, t_data, a_data, a_full, "orange")

        # [요청사항] 상자 위치 및 텍스트 (좌표 엄수)
        ...
        fig.update_layout(
            height=850, margin=dict(l=20, r=20, t=30, b=150), showlegend=False,
        ...
        st.plotly_chart(fig, use_container_width=True, config={'displayModeBar': False}, key=f"chart_{st.session_state.is_playing}")
