...
if page == "1차원 운동":
    ...
    with col_main:
        ...
        # 서브플롯 구성 (가속도-시간 그래프 제목 삭제)
        fig = make_subplots(
            rows=2, cols=3, 
            specs=[[{"colspan": 3}, None, None], [{}, {}, {}]],
            subplot_titles=("", "■ 위치-시간 그래프", "■ 속도-시간 그래프", ""), 
            row_heights=[0.3, 0.7],
    ...
    with tabs[0]:
        ...
        t = st.slider("시간(s)", 0.0, 3.0, 1.5, key="f_v14")
        fig = go.Figure()
    ...
    with tabs[1]:
        ...
        t = st.slider("시간(s)", 0.0, 3.0, 1.5, key="p_v14")
        fig = go.Figure()
    ...
    with tabs[2]:
        ...
        ang = st.slider("각도(도)", 0, 360, 45, key="c_v14")
        r = np.radians(ang)
        fig = go.Figure()
